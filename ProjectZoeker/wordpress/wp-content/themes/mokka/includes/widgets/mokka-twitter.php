...
class Mokka_Twitter extends WP_Widget {
	...
	**/
	public function __construct() {
		...
		parent::__construct(
	 		'mokka_twitter', // Base ID
			__( 'Mokka: Twitter Feeds',  'favethemes'  ), // Name
		...
		<?php echo $after_title; 
            	if( !isset( $error ) && is_array($twitterData)){
		            ...
		            foreach($twitterData as $item){
		                    ...
		                    $link = $permalink;
		                     echo '<div class="tweet"><p><i class="icon icon-socialmedia-07"></i>';
		                    ...
		                    if($update) {
		                      ...
		                      echo sprintf(esc_html('%s', 'twitter-for-wordpress'),' <p class="tweet-time"><abbr title="' . date(esc_html('Y/m/d H:i:s'), $time) . '">' . $h_time . '</abbr></p>' );
		                     }
		                    echo '</div>';
		                    $i++;
